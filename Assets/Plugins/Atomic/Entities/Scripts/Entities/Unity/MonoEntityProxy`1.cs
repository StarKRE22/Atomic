...
namespace Atomic.Entities
{
    ...
    /// </summary>
    /// <typeparam name="E">The type of the source entity, must inherit from <see cref="MonoEntity"/>.</typeparam>
    /// <remarks>
    ...
    /// you can place <c>MonoEntityProxy</c> on each child and reference the same source <see cref="MonoEntity"/>.
    ...
    /// {
    ///     if (other.TryGetComponent(out IEntity proxy)) // concrete type is MonoEntityProxy
    ///     {
    ...
    /// </example>
    /// </remarks>
    [HelpURL("https://github.com/StarKRE22/Atomic/blob/main/Docs/Entities/Entities/MonoEntityProxy%601.md")]
    public abstract class MonoEntityProxy<E> : MonoBehaviour, IEntity where E : MonoEntity
    {
        ...
        public ref T GetValueUnsafe<T>(int key) where T : class => ref _source.GetValueUnsafe<T>(key);
        ...
        public bool TryGetValueUnsafe<T>(int key, out T value) where T : class => _source.TryGetValueUnsafe(key, out value);

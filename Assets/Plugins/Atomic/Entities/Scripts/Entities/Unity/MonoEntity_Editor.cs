...
{
    /// <summary>
    /// Provides editor-time lifecycle support for the <see cref="MonoEntity"/>,
    ...
    public partial class MonoEntity
    {
        ...
        private void SetRefreshCallbackToInstallers()
        {
            for (int i = 0, count = this.sceneInstallers.Count; i < count; i++)
            {
                ...
                if (installer != null)
                    installer.onValidateCallback = this.Compile;
            }
        }
        ...
        private void InitInEditMode()
        {
            ...
            for (int i = 0; i < _behaviourCount; i++)
            {
                ...
                if (behaviour is IEntityInit dispose && IsEditModeBehaviourDefined(behaviour))
                    dispose.Init(this);
            }
        ...
        private void EnableInEditMode()
        {
            ...
            {
                IEntityBehaviour behaviour = _behaviours[i];
                if (behaviour is IEntityEnable dispose && IsEditModeBehaviourDefined(behaviour))
                    dispose.Enable(this);
            }
        ...
        private void DisableInEditMode()
        {
            ...
            {
                IEntityBehaviour behaviour = _behaviours[i];
                if (behaviour is IEntityDisable disable && IsEditModeBehaviourDefined(behaviour))
        ...
        private void DisposeInEditMode()
        {
            ...
            {
                IEntityBehaviour behaviour = _behaviours[i];
                if (behaviour is IEntityDispose dispose && IsEditModeBehaviourDefined(behaviour))
        ...
        private protected virtual void Reset()
        {
            ...
            this.sceneInstallers = new List<MonoEntityInstaller>(this.GetComponentsInChildren<MonoEntityInstaller>());

#if UNITY_5_3_OR_NEWER
using System;
using UnityEngine;

namespace Atomic.Elements
{
    /// <summary>
    /// Reactive wrapper around a <see cref="UnityEngine.Transform"/> rotation that
    /// exposes an <see cref="IReactiveVariable{T}"/>-like API.
    /// </summary>
    /// <remarks>
    /// - Reading <see cref="Value"/> returns the cached rotation.
    /// - Setting <see cref="Value"/> updates the cache, assigns <see cref="Transform.rotation"/>,
    ///   and notifies subscribers via <see cref="OnValueChanged"/> (only when the value actually changes).
    /// - Use <see cref="Subscribe"/> / <see cref="Unsubscribe"/> to observe changes,
    ///   or dispose subscriptions via returned <see cref="Subscription{T}"/>.
    /// </remarks>
    public sealed class TransformRotationVariable : IReactiveVariable<Quaternion>, IDisposable
    {
        /// <summary>
        /// Raised after <see cref="Value"/> changes and the underlying
        /// <see cref="Transform.rotation"/> is updated.
        /// </summary>
        public event Action<Quaternion> OnValueChanged;

        /// <summary>
        /// Gets or sets the current rotation value.
        /// </summary>
        /// <remarks>
        /// Setter performs a change check:
        /// if the new value equals the cached rotation, no assignment or notification occurs.
        /// Otherwise, the internal cache is updated, <see cref="Transform.rotation"/> is set,
        /// and <see cref="OnValueChanged"/> is invoked.
        /// </remarks>
        public Quaternion Value
        {
            get { return _rotation; }
            set
            {
                if (_rotation != value)
                {
                    _rotation = value;
                    _transform.rotation = value;
                    this.OnValueChanged?.Invoke(value);
                }
            }
        }

        /// <summary>
        /// Backing <see cref="UnityEngine.Transform"/> whose rotation is controlled by this wrapper.
        /// </summary>
        private readonly Transform _transform;

        /// <summary>
        /// Cached rotation used to avoid redundant notifications and assignments.
        /// </summary>
        private Quaternion _rotation;

        /// <summary>
        /// Creates a new <see cref="TransformRotationVariable"/> bound to the provided <see cref="Transform"/>.
        /// </summary>
        /// <param name="transform">Target transform. Must not be <c>null</c>.</param>
        /// <exception cref="ArgumentNullException">Thrown if <paramref name="transform"/> is <c>null</c>.</exception>
        public TransformRotationVariable(Transform transform)
        {
            _transform = transform ?? throw new ArgumentNullException(nameof(transform));
            _rotation = _transform.rotation;
        }

        /// <summary>
        /// Subscribes to value change notifications.
        /// </summary>
        /// <param name="action">Callback invoked with the new value after it changes.</param>
        /// <returns>
        /// A <see cref="Subscription{T}"/> that can be disposed to unsubscribe.
        /// </returns>
        public Subscription<Quaternion> Subscribe(Action<Quaternion> action)
        {
            this.OnValueChanged += action;
            return new Subscription<Quaternion>(this, action);
        }

        /// <summary>
        /// Unsubscribes a previously registered callback.
        /// </summary>
        /// <param name="action">The callback to remove.</param>
        public void Unsubscribe(Action<Quaternion> action) => this.OnValueChanged -= action;

        /// <summary>
        /// Releases all subscribers by clearing <see cref="OnValueChanged"/>.
        /// </summary>
        /// <remarks>
        /// After disposal, further <see cref="Value"/> updates will no longer notify any listeners.
        /// </remarks>
        public void Dispose() => this.OnValueChanged = null;
    }
}
#endif
#if UNITY_5_3_OR_NEWER
using System;
using Sirenix.OdinInspector;
using UnityEngine;

namespace Atomic.Elements
{
    /// <summary>
    /// A pointer wrapper for invoking a <see cref="MonoAction"/> from outside the scene context.
    /// </summary>
    /// <remarks>
    /// This class allows referencing and triggering a <see cref="MonoAction"/> as a serializable field,
    /// often useful in serialized action chains or as part of a logic asset.
    /// </remarks>
    [Serializable]
    public sealed class MonoActionProxy : IAction
    {
        /// <summary>
        /// Reference to the actual <see cref="MonoAction"/> component to be invoked.
        /// </summary>
        [SerializeField]
#if ODIN_INSPECTOR
        [SceneObjectsOnly, Required]
#endif
        private MonoAction action;

        /// <summary>
        /// Default constructor.
        /// </summary>
        public MonoActionProxy()
        {
        }

        /// <summary>
        /// Creates a new <see cref="MonoActionProxy"/> with the specified scene action.
        /// </summary>
        /// <param name="action">The <see cref="MonoAction"/> to wrap.</param>
        public MonoActionProxy(MonoAction action) => this.action = action;

        /// <summary>
        /// Assigns a <see cref="MonoAction"/> to this pointer.
        /// </summary>
        /// <param name="action">The scene action to assign.</param>
        public void Construct(MonoAction action) => this.action = action;

        /// <summary>
        /// Invokes the referenced <see cref="MonoAction"/>, if it exists.
        /// </summary>
        public void Invoke()
        {
            if (this.action)
                this.action.Invoke();
        }
    }
}
#endif